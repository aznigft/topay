...
const BankAccounts = ({setNavState}) => {
  ...
  const emptyBankAccount = {id:'', name: '', number: '', bankName: '', visibleToOthers: true}

  const [MenuOptions, setMenuOptions] = React.useState(AccountOptions);
  const [myProfile, setMyProfile] = React.useState(null);
  const [selectedBankAccount, setSelectedBankAccount] = React.useState(emptyBankAccount)
